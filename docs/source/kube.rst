Scaling [iotsolution-restapi-3f10] With Kubernetes
===========================
...
Based on your TML solution [iotsolution-restapi-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`iotsolution-restapi-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-iotsolution-restapi-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingressiotsolution-restapi-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f iotsolution-restapi-3f10.yml

iotsolution-restapi-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: iotsolution-restapi-3f10.yml - and save it locally.
...
.. code-block:: YAML
   ...
   ################# iotsolution-restapi-3f10.yml
     ...
     kind: Deployment
     metadata:
       name: iotsolution-restapi-3f10
     spec:
       selector:
         matchLabels:
           app: iotsolution-restapi-3f10
       ...
       template:
         ...
         spec:
           containers:
           - name: iotsolution-restapi-3f10
             image: maadsdocker/iotsolution-restapi-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: SOLUTIONNAME
               value: 'iotsolution-restapi-3f10'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag_restapi-iotsolution-restapi-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
   ...
   ---
     ...
     metadata:
       ...
       labels:
         app: iotsolution-restapi-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
   ...
   ---
     ...
     metadata:
       ...
       labels:
         app: iotsolution-restapi-3f10-external-service
     spec:
       type: ClusterIP
       ports:
       - port: 80 # Ingress port, if using port 443 will need to setup TLS certs
...
.. code-block::
   ...
   kubectl port-forward deployment/iotsolution-restapi-3f10 40847:40847
